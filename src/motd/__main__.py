    ...
    try:
        ...
        for i, match in enumerate(result.matches, 1):
            ...
            if match.venue_result and match.clustering_result:
                ...
                # Only calculate agreement if both have valid timestamps
                if venue_ts is not None and cluster_ts is not None:
        ...
        # Generate diagnostic JSON if debug mode enabled
        if debug:
            click.echo(f"{'='*60}")
            ...
            diagnostics = {
                ...
                "ground_truth": GROUND_TRUTH_INTROS,
                "matches": []
            }
            ...
            for i, match in enumerate(result.matches, 1):
                ...
                if match.clustering_result:
                    if 'diagnostics' in match.clustering_result:
                        ...
                        elif ground_truth and match.clustering_result.get('timestamp'):
                            cluster_ts = match.clustering_result['timestamp']
                            diff = abs(cluster_ts - ground_truth)
                            if diff <= 10:
            ...
            # Save debug JSON
            debug_output = Path(f'data/output/{episode_id}/clustering_debug.json')
